// 新闻来源
let source_name = "新浪新闻";
// 编码格式
let myEncoding = "utf-8";
// 主页URL
let seedURL = 'https://news.sina.com.cn/';
let myURL = "";
// Jquery全局对象
let seedURL_format = "$('*')";
...
request(seedURL, function(err, res, body) {
    ...
    seedurl_news.each(function(i, e) {
        try {
            ...
            if (
                href.match(/https:\/\/news.sina.com.cn.*?shtml/)
...
function getHotNews(res, myURL) {
    ...
    fetch.content = $(".article").eq(0).text().replace(/\s*/g,"");

    fetch.keywords = $('meta[name=\"keywords\"]').eq(0).attr("content");

    fetch.publish_date = $('meta[name=\"weibo: article:create_at\"]').eq(0).attr('content');

    fetch.author = $('.show_author').eq(0).text().replace(/\s*/g,"");
    ...
    // 输出为json文件
    fs.writeFileSync('/res'+filename, JSON.stringify(fetch));
     */
