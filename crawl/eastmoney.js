// 新闻来源
let source_name = "东方财富";
// 编码格式
let myEncoding = "utf-8";
// 主页URL
let seedURL = 'https://www.eastmoney.com/';
let myURL = "";
// Jquery全局对象
let seedURL_format = "$('*')";
...
request(seedURL, function(err, res, body) {
    ...
    seedurl_news.each(function(i, e) {
        try {
            ...
            if (
                href.match(/https:\/\/finance.eastmoney.com\/a\/.*?html/)
...
function getHotNews(res, myURL) {
    ...
    fetch.title = $('.topbox').children('.title').text();//ok

    fetch.content = $(".txtinfos").clone().children('.infokey').end().text().replace(/\s*/g,"");

    fetch.keywords = $('meta[name=\"keywords\"]').eq(0).attr("content");

    fetch.publish_date = $('.tipbox').children('.infos').children().eq(0).text().substring(0,10).replace("年",'-').replace("月",'-').replace("日",'-');

    fetch.author = $('.sourcebox').children('.flr').text().replace(/\s*/g,"");

    fetch.desc = $('meta[name=\"description\"]').eq(0).attr("content");//ok
